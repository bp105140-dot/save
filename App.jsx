...
// ─── TTS ───────────────────────────────────────────────────────────────────────
// Truque iOS/Android: um único elemento <audio> criado no primeiro gesto do
// usuário fica "desbloqueado" para sempre — mesmo para plays assíncronos depois.
// Funciona porque mantemos o MESMO elemento e só trocamos o src.

const audioCache = new Map(); // text -> blob URL
let persistentAudio = null;   // elemento único desbloqueado no primeiro toque

function unlockAudio() {
  if (persistentAudio) return;
  // Cria o elemento dentro do gesto — iOS marca como "trusted"
  persistentAudio = new Audio();
  persistentAudio.volume = 1;
  // Silêncio mínimo pra confirmar desbloqueio sem ruído
...
async function speakElevenLabs(text) {
  try {
    ...
    // Reutiliza o elemento desbloqueado — iOS/Android permitem mesmo após await
